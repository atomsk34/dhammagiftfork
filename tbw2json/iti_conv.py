...
def is_separator(tag):
    """
    Определяет, является ли тег разделителем. Разделителями считаются только заголовки.
    Тег <hr> удален из списка, так как он используется непоследовательно в исходном файле.
    """
...
def split_into_blocks(content_div):
    """
    Разделяет контент на блоки. Каждый блок начинается с тега-разделителя (h1-h6).
    """
    blocks = []
    current_block = []
    # Используем find_all для надежного поиска всех релевантных тегов
    tags_to_process = content_div.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'hr', 'p'])
...
def process_sutta(html_filepath, output_dir="output"):
    ...
    for i in range(max_blocks):
        ...
        for en_tags, pi_tags in aligned_rows:
            ...
            # Для английского текста используется перевод Thanissaro, как в имени файла
    ...
    # Исправляем имя файла в соответствии с предоставленным JSON
    en_filename = os.path.join(output_dir, f"{sutta_id}_translation-en-thanissaro.json")
...
if __name__ == "__main__":
    # Замените 'it.html' на имя вашего файла, если оно отличается
    filepath = 'it.html'
    if not os.path.exists(filepath):
        print(f"HTML файл '{filepath}' не найден в текущей директории.")
    else:
        print(f"Найден 1 HTML файл для обработки: {filepath}")
        try:
